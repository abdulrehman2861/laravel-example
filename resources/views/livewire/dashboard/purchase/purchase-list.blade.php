                ...
                <tbody>
                    ...
                    @forelse ($purchases as $purchase)
                        <tr>
                            ...
                            <td><span
                                    class="badge @if ($purchase->status == 'Pending') badge-info @elseif($purchase->status == 'Ordered' || $purchase->status == 'Completed') badge-success @endif ">
                                    {{ $purchase->status ?? '-' }} </span> </td>
                            <td>{{ $purchase->total_amount }}</td>
                            <td>{{ $purchase->amount_paid }}</td>
                            <td>{{ $purchase->total_amount - (float) $purchase->amount_paid }}</td>
                            <td><span
                                    class="badge @if ($purchase->payment_status == 'Unpaid') badge-danger @elseif($purchase->payment_status == 'Returned' || $purchase->payment_status == 'Paid') badge-success @endif ">
                                    {{ $purchase->payment_status ?? '-' }} </span> </td>
                            <td>{{ $purchase->shippedWarehouse?->name }}</td>
                            <td>

                                @can('edit_purchases')
                                    <a class="btn btn-primary btn-xs"
                                        href="{{ route('dashboard.purchase.edit', $purchase->id) }}"><span
                                            class="fas fa-pen"></span></a>
                                @endcan
                                @can('show_purchases')
                                    <a class="btn btn-info btn-xs"
                                        href="{{ route('dashboard.purchase.show', $purchase->id) }}"><span
                                            class="fas fa-eye"></span></a>
                                @endcan

                                @can('delete_purchases')
                                    <button
                                        onclick="confirmDelete(`{{ route('dashboard.purchase.destroy', $purchase->id) }}`)"
