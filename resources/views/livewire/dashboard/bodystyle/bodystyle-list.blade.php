        ...
        <div class="col-md-12 ">
            ...
            <table wire:loading.remove id="my-all-items-table" class="table table-striped table-bordered"
                ...
                <tbody>
                    ...
                    @forelse ($bodyStyles as $bodyStyle)
                        <tr>
                            <td>{{ $bodyStyle->name }}</td>
                            <td>{{ $bodyStyle->cars->count() }}
                            <td>{{ $bodyStyle->productFitting->pluck('product')->unique()->count() }}
                            </td>

                            <td>

                                <a class="btn btn-primary btn-xs"
                                    href="{{ route('dashboard.bodystyle.edit', $bodyStyle->id) }}"><span
                                        class="fas fa-pen"></span></a>


                                <button
                                    onclick="confirmDelete(`{{ route('dashboard.bodystyle.destroy', $bodyStyle->id) }}`)"
