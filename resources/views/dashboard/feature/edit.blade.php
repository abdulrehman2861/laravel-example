        ...
        <div class="card w-50 mx-auto" role="document">
            <div class="">
                <div class="modal-header">
                    <h3 class="modal-title" id="exampleModalLabel">Update Feature</h3>
                </div>

                <form method="POST" action="{{ route('dashboard.feature.update', $feature->id) }}">
                    <div class="modal-body">
                        @csrf

                        <div class="form-group">
                            <label for="name" class="col-form-label">feature *</label>
                            <input type="text" class="form-control" name="name" value="{{$feature->name}}" id="name" required>

                        </div>

                        <div class="form-group">
                            <label for="message-text" class="col-form-label">Glass *</label>


                            <select class="form-control " name="glass_id">
                                <option selected="true">
                                    Select Glass
                                </option>

                                @forelse ($glasses as $glass)
                                    <option value="{{ $glass->id }}" @if($glass->id == $feature->glass_id) selected="true"  @endif >
                                        {{ $glass->name }}
                                    </option>
                                @empty
