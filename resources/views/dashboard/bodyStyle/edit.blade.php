        ...
        <div class="card w-50 mx-auto" role="document">
            <div class="">
                <div class="modal-header">
                    <h3 class="modal-title" id="exampleModalLabel">Update Bodystyle</h3>
                </div>

                <form method="POST" action="{{ route('dashboard.bodystyle.update', $bodyStyle->id) }}">
                    <div class="modal-body">
                        @csrf

                        <div class="form-group">
                            <label for="name" class="col-form-label">Bodystyle *</label>
                            <input type="text" class="form-control" name="name" value="{{$bodyStyle->name}}" id="name" required>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control " name="car_id">
                                ...
                                @forelse ($models as $model)
                                    <option value="{{ $model->id }}" @if($model->id == $bodyStyle->cars()->first()->id ?? 0) selected="true"  @endif>
                                        {{ $model->name }}
                                    </option>
                                @empty
