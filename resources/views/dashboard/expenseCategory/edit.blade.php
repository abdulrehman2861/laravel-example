        ...
        <div class="card w-50 mx-auto" role="document">
            <div class="">
                <div class="modal-header">
                    <h3 class="modal-title" id="exampleModalLabel">Update Expense Category</h3>
                </div>

                <form method="POST" action="{{ route('dashboard.expenseCategory.update', $expenseCategory->id) }}">
                    <div class="modal-body">
                        @csrf

                        <div class="form-group">
                            <label for="name" class="col-form-label">Category Name *</label>
                            <input type="text" name="name" class="form-control" value="{{$expenseCategory->name}}"  placeholder="Enter Expense category" id="name" required>
                        </div>
                        <div class="form-group">
                            <label for="description" class="col-form-label">Description *</label>
                            <textarea id="description" class="form-control" name="description" cols="50" rows="4" placeholder="Enter description">{{$expenseCategory->description}}</textarea>
                        </div>
                    </div>
                    </div>
