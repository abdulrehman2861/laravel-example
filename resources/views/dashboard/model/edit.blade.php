        ...
        <div class="card w-50 mx-auto" role="document">
            <div class="">
                <div class="modal-header">
                    <h3 class="modal-title" id="exampleModalLabel">Update Model</h3>
                </div>

                <form method="POST" action="{{ route('dashboard.model.update', $model->id) }}">
                    <div class="modal-body">
                        @csrf

                        <div class="form-group">
                            <label for="name" class="col-form-label">Model *</label>
                            <input type="text" class="form-control" name="name" value="{{$model->name}}" id="name" required>

                        </div>

                        <div class="form-group">
                            <label for="message-text" class="col-form-label">Make *</label>


                            <select class="form-control " name="car_company_id">
                                <option >
                                    Select Make
                                </option>

                                @forelse ($makes as $make)
                                    <option value="{{ $make->id }}" @if($make->id == $model->car_company_id) selected="true"  @endif>
                                        {{ $make->name }}
                                    </option>
                                @empty
