        ...
        <div class="card w-50 mx-auto" role="document">
            <div class="">
                <div class="modal-header">
                    <h3 class="modal-title" id="exampleModalLabel">Update warehouse</h3>
                </div>

                <form method="POST" action="{{ route('dashboard.warehouse.update', $warehouse->id) }}">
                    <div class="modal-body">
                        @csrf

                        <div class="form-group">
                            <label for="location_code" class="col-form-label">Location Code *</label>
                            <input type="text" class="form-control" name="location_code" value="{{$warehouse->name}}" id="location_code" required>

                        </div>

                        <div class="form-group">
                            <label for="name" class="col-form-label">Name *</label>
                            <input type="text" class="form-control" name="name" value="{{$warehouse->name}}" id="name" required>

                        </div>

                        <div class="form-group">
                            <label for="address" class="col-form-label">Address *</label>
                            <textarea type="text" name="address" class="form-control" id="address" required>{{$warehouse->address}}</textarea>
                        </div>
