...
class SaleReturnCreate extends Component
{
    ...
    public function boot(CustomerRepositoryInterface $customerRepository)
    {
        ...
        $this->suppliers = $this->customerRepository->allCustomersNonPaginated();
    }
    ...
    {
        return view('livewire.dashboard.sale-return.sale-return-create');
    }
    ...
    public function selectTransaction($id)
    {
        ...
        $tmpTransaction = SaleTransaction::find($id);
        if ($tmpTransaction) {
    ...
    public function getProducts()
    {
        ...
        $query->when(isset($this->generalKey) && !empty($this->generalKey), function ($q) {
            ...
            $q->where(function ($q) {
                    ...
                    ->orWhere(DB::raw("CONCAT('QT-', LPAD(id, 3, '0'))"), 'LIKE', "%{$this->generalKey}%");
            });
        });
        ...
        $query->when(isset($this->searchSupplier) && !empty($this->searchSupplier), function ($q) {
            ...
            $q->where('customer_id', $this->searchSupplier);
        });

        $this->transactionList = $query->paginate(10, ['*'], 'page', $this->transactionList?->currentPage() ?? 0 + 1);

        $this->transactions = SaleTransaction::whereIn('id', $this->transactionList->pluck('id')->toArray())->get();
    }
